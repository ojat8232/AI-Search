...
my_first_name = "Oliver"
my_last_name = "Thompson"
...
class edge:
    def __init__(self, node1, node2, length):
        ...
        self.length = length
        self.pheremone = length * startDeposit

class ant:
    def __init__(self):
        self.startLocation = random.randint(0, num_cities - 1)
    ...
    def updateLocation(self, next):
        ...
        self.distance = self.distance + length(next, self.location)
        self.location = next
...
def pheremone(a, b):
    link = findEdge(a, b)
    return link.pheremone

def length(a, b):
    link = dist_matrix[a][b]
    return link

def nextNode(ant):
    global sensitvity
    if len(ant.route) == num_cities:
        return ant.startLocation

    pheremones = []
    for x in range(0, num_cities):
        if (not (x == ant.startLocation)) and (not (x == ant.location)) and (not(x in ant.route)):
            pheremones.append((pheremone(ant.location, x)*sensitvity) + 1)
        else:
            pheremones.append(0)
    value = random.randint(0, sum(pheremones))

    for y in range(0, num_cities):
        if value <= pheremones[y] and pheremones[y] != 0:
            return y
        else:
            value = value - pheremones[y]

def routeLength(ant):
    routeLength = 0

    for x in range(0, len(ant.route) - 1):
        routeLength = routeLength + length(ant.route[x], ant.route[x + 1])
    return routeLength

def layPheremone(ant):
    addPher = int(routeLength(ant) / layAmount)

    for x in range(0, len(ant.route) - 1):
        a, b = returnEdge(ant.route[x], ant.route[x + 1])
        edges[a][b].pheremone = edges[a][b].pheremone + addPher

def decayPheremone():
    for edgeList in edges:
        for edge in edgeList:
            edge.pheremone = int(edge.pheremone * decayRate)

def resetAnt(ant):
    ant.route.clear()
    ant.route.append(ant.startLocation)
    ant.distance = 0
...
def iteration():
    ...
    for ant in ants:
        if (time.time() - startTime) > 55:
            break
        if ant.distance < tour_length or tour == []:
            tour.clear()
            for x in range(0, num_cities):
                tour.append(ant.route[x])
            tour_length = routeLength(ant)
            #print(tour_length)
        layPheremone(ant)
        resetAnt(ant)
    elitism()
...
def elitism():
    for x in range(0, len(tour) - 1):
        a, b = returnEdge(tour[x], tour[x + 1])
        edges[a][b].pheremone = edges[a][b].pheremone + elite
...
for x in range(0, num_cities - 1):
    ...
    for y in range(x + 1, num_cities):  
        listOfEdges.append(edge(x, y, dist_matrix[x][y]))
    edges.append(listOfEdges)


ants = []
for x in range(0, antNumber):
    ants.append(ant())
