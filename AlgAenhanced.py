...
def mutate(child):
    mutate = 1
    while mutate == 1:
        a = random.randint(0, mutationRate)
        if a == 0:         
            a = random.randint(0, len(child) - 1)
            b = random.randint(0, len(child) - 1)
            t = child[a]
            child[a] = child[b]
            child[b] = t
        else:
...
def fitness(chromosone):
    ...
    for x in range(0, num_cities - 1):
        fit = fit + dist_matrix[chromosone[x]][chromosone[x + 1]]
    fit = fit + dist_matrix[chromosone[num_cities - 1]][chromosone[0]]
    return fit

def breed(parentA, parentB):
    s = random.randint(0, len(parentA))
    ...
    childA = duplicates(childA)
    childB = duplicates(childB)

    childA = mutate(childA)
    childB = mutate(childB)

    totalA = fitness(childA)
    totalB = fitness(childB)
...
def duplicates(child):
    genes = [None] * num_cities
    duplicates = []
    missing = []

    #finds duplicate cities
    for x in range(0, num_cities):
        if genes[child[x]] == None:
            genes[child[x]] = x

        else:
            duplicates.append([x, genes[child[x]]])
    
    #finds missing cities
    for y in range(0, num_cities):
        if genes[y] == None:
            missing.append(y)

    #replaces duplicates with missing cities
    for duplicate in duplicates:
        n = random.randint(0, len(missing) - 1)
        z = random.randint(0, 1)
        if z == 0:
            child[duplicate[0]] = missing[n]
        else:
            child[duplicate[1]] = missing[n]
        missing.pop(n)
    return child      
...
def initialise():
    population = []
    for x in range(0, initialPopulation):
        population.append(createLife())
    return population
...
def roulette(population):
    ...
    for chromosone in population:
        fit = 0
        #print(fitness(chromosone))
        fit = 1 / (fitness(chromosone) ** 50)
    ...
    for fit in fitnesses:
        ...
        #print("overallFit = " + str(overallFit))
        sector = ((fit * 360) / totalFitness)
        #print("sector = " + str(sector))
        #print()
        wheel.append(sector + overallFit)
        overallFit = overallFit + sector

    wheel[len(wheel) - 1] = 360


    children = []
    for x in range(0, len(population)):
        parentA = population[roll(wheel)]
        #print(parentA)

        parentB = population[roll(wheel)]
        #print(parentB)
...
tour = population[0]
tour_length = fitness(population[0])

for x in range(0, iterations):
    population = roulette(population)
    
    for chromosone in population:
        if fitness(chromosone) < tour_length:
            tour_length = fitness(chromosone)
            tour = chromosone
            #print(fitness(chromosone))
